...
import (
	...
	"github.com/krateoplatformops/crdgen/v2/internal/utils/bufferpool"
)
...
func String(format string, args ...any) *istring {
	if len(args) == 0 {
		x := istring(format)
		return &x
	}
...
func LineComment(content string, args ...any) *istring {
	if len(args) != 0 {
...
func Template(data any, tmpl string) Node {
	buf := pool.Get()
	defer buf.Free()
